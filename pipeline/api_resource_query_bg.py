...
"""
    ...
    # )
    iserror = False
    if len(api_response) == 0 or api_request.status_code != 200:
        iserror = True
    response_type = target_format if target_format != "" else response_type
    if response_type.lower() == "json" and len(errors) == 0 and not iserror:
    ...
    if response_type.lower() == "csv" and len(errors) == 0 and not iserror:
        print(api_response)
    ...
    if response_type.lower() == "xml" and len(errors) == 0 and not iserror:
    ...
    if (response_type.lower() not in ["xml", "csv", "json"] and len(errors) == 0) or iserror:
        with open(file_name + "-data.xml", "w") as f:
