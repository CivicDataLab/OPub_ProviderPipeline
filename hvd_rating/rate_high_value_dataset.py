...
HVD_NOTE = f'''Methodology for calculating ratings for each header of this report is as follows
...
def get_rating_and_update_dataset(params_json=default_params_json):
    ...
    for dataset in all_datasets:
        ...
        #    break
    num_of_datasets = len(dataset_ids)
    ...
    flow_start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    hvd_count = 0
    for dataset_id in dataset_ids:
        ...
        if round(sum(rating_list), 1) >= HVD_THRESHOLD:
            hvd_count += 1
    flow_end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    extras = {
        "analysis_start_time": flow_start_time,
        "analysis_end_time": flow_end_time,
        "total_datasets_analysed": num_of_datasets,
        "total_high_value_datasets_identified": hvd_count
    }
    print("Extras!!!", extras)
    ...
    try:
        ...
        response = requests.request("POST", email_url, headers=headers, files=files, params=extras)
